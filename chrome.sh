...
# 获取 xpra 使用的 Python 解释器路径
_chrome::xpra_python() {
    local xpra_bin
    xpra_bin="$(command -v xpra 2>/dev/null)" || { echo "python3"; return; }

    # xpra 脚本首行通常包含 #!/usr/bin/python3.x 或 #!/usr/bin/env python3
    local shebang
    shebang="$(head -1 "$xpra_bin" 2>/dev/null)" || { echo "python3"; return; }

    local py_cmd
    py_cmd="$(echo "$shebang" | sed -n 's|^#!.*\(python[0-9.]*\)|\1|p')" || true
    if [[ -n "$py_cmd" ]] && command -v "$py_cmd" &>/dev/null; then
        echo "$py_cmd"
    else
        echo "python3"
    fi
...
# 修补 xpra GTK 剪贴板模块的已知 bug
# 某些发行版打包的 xpra 版本中，clipboard.py 从 proxy 独立导入 filter_data，
# 但该函数仅作为类方法存在，导致 ImportError。此 bug 在 xpra 6.4.3 中已修复。
_chrome::patch_clipboard_if_needed() {
    local py_cmd
    py_cmd="$(_chrome::xpra_python)"

    # 测试是否能正常导入
    if "$py_cmd" -c "from xpra.gtk.clipboard import GTK_Clipboard" &>/dev/null; then
        return 0
    fi

    # 定位 clipboard.py 文件（覆盖各发行版的 Python 包安装路径）
    local py_ver
    ...
    local candidates=(
        ...
        "/usr/lib64/python${py_ver}/site-packages/xpra/gtk/clipboard.py"
        "/usr/lib/python3/dist-packages/xpra/gtk/clipboard.py"
        "/usr/local/lib/python${py_ver}/dist-packages/xpra/gtk/clipboard.py"
    )
    for f in "${candidates[@]}"; do
        [[ -f "$f" ]] && clipboard_py="$f" && break
    done

    # 如果上述路径都找不到，通过 Python 自身定位
    if [[ -z "$clipboard_py" ]]; then
        clipboard_py="$("$py_cmd" -c "import xpra.gtk; print(xpra.gtk.__file__.replace('__init__.py','clipboard.py'))" 2>/dev/null)" || true
        [[ -f "$clipboard_py" ]] || clipboard_py=""
    fi

    if [[ -z "$clipboard_py" ]]; then
        log::warn "无法定位 xpra 剪贴板模块文件，剪贴板可能不可用"
        return 0
    fi

    # 确认是已知的 filter_data import bug
    if ! grep -q 'import ClipboardProxyCore, filter_data' "$clipboard_py" 2>/dev/null; then
        log::warn "xpra 剪贴板模块异常但非已知 bug，剪贴板可能不可用"
        return 0
    fi

    log::info "检测到 xpra 剪贴板模块已知 bug，自动修补中..."
    sudo::ensure
    ...
    # 清理 .pyc 缓存，确保下次加载使用修改后的源文件
    local xpra_pkg_dir
    xpra_pkg_dir="$(dirname "$(dirname "$clipboard_py")")"
    sudo::exec find "$xpra_pkg_dir" -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null || true
...
# 确保宿主机安装了与 VM 端版本兼容的 xpra 及剪贴板依赖
_chrome::ensure_xpra() {
    local current_major
    ...
    if (( current_major >= _XPRA_MIN_MAJOR )); then
        ...
        _chrome::install_xpra_from_official
        log::ok "xpra 升级完成 (v$(xpra --version 2>/dev/null | grep -oE '[0-9]+\.[0-9.]+' | head -1))"
    else
    ...
    # xpra 剪贴板功能依赖 python-dbus 和剪贴板工具
    local py_cmd
    py_cmd="$(_chrome::xpra_python)"

    local need_deps=()
    if ! "$py_cmd" -c "import dbus" &>/dev/null; then
        need_deps+=("python-dbus")
    fi
    if ! command -v xclip &>/dev/null; then
        need_deps+=("xclip")
    fi
    # Wayland 环境额外需要 wl-clipboard 提供原生剪贴板支持
    if [[ "${XDG_SESSION_TYPE:-}" == "wayland" ]] || [[ -n "${WAYLAND_DISPLAY:-}" ]]; then
        if ! command -v wl-copy &>/dev/null; then
            need_deps+=("wl-clipboard")
        fi
    fi

    if [[ ${#need_deps[@]} -gt 0 ]]; then
        log::info "安装 xpra 剪贴板依赖: ${need_deps[*]}..."
        sudo::ensure
        pkg::install "${need_deps[@]}"
        log::ok "剪贴板依赖安装完成"
    fi
...
# 清理 VM 端残留的旧 xpra 会话（版本升级后旧进程仍在运行会导致协议不兼容）
_chrome::cleanup_stale_sessions() {
    local user="$1" host="$2" key="$3"
    local ssh_opts=(-i "$key" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=5 -o LogLevel=ERROR)

    local sessions
    sessions="$(ssh "${ssh_opts[@]}" "${user}@${host}" \
        "xpra list 2>/dev/null | grep -c LIVE" 2>/dev/null)" || sessions="0"

    if (( sessions > 0 )); then
        log::info "清理 VM 端残留的 xpra 会话..."
        ssh "${ssh_opts[@]}" "${user}@${host}" \
            'xpra stop --all 2>/dev/null; pkill -u $(whoami) -f "xpra" 2>/dev/null || true' 2>/dev/null || true
        sleep 2
    fi
}
...
# 清理 VM 端可能残留的旧 xpra 会话
_chrome::cleanup_stale_sessions "$VM_USER" "$ip" "$SSH_KEY_PATH"
